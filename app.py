...
def health_check():
    ...
    return {"status": "healthy", "service": "telos-whatsapp-bot"}, 200
...
def whatsapp_webhook():
    ...
    try:
        ...
        if not db:
            resp.message("Sorry, Telos is temporarily unavailable. Please try again later.")
            return str(resp)

        # Handle empty message
        if not body:
            resp.message("👋 Welcome to *Telos* - Your Business Directory!\n\n🔍 *Search*: Send keywords like 'pizza', 'hotel'\n📝 *Register*: Send 'register' to add your business\n❓ *Help*: Send 'help' for more options")
            return str(resp)
        ...
        if body_lower in ['help', 'start', 'menu']:
            help_message = """🏢 *Telos Business Directory*
...
🌟 *Telos* - Connecting businesses with customers!"""
        ...
        resp = MessagingResponse()
        resp.message("Sorry, Telos encountered an issue. Please try again or send 'help' for assistance.")
        return str(resp)
...
def start_registration(from_number, resp):
    ...
    resp.message("""📝 *Telos Business Registration*

Welcome to Telos! Let's add your business to our directory.
...
🌟 Join thousands of businesses on Telos!""")
...
def handle_registration_step(from_number, body, resp):
    ...
    if body.lower() == 'cancel':
        ...
        resp.message("❌ Registration cancelled.\n\nSend 'register' to start again or 'help' for other options.\n\n🌟 Telos is here whenever you're ready!")
        return str(resp)
...
💡 Include area/neighborhood for better visibility on Telos!""")
...
📞 This helps Telos customers contact you directly.""")
...
Separate with commas. This helps customers find you on Telos!
...
💡 Include 3-8 relevant keywords for best results.""")
...
def complete_registration(from_number, resp):
    ...
    try:
        ...
        # Send confirmation
        confirmation = f"""🎉 *Welcome to Telos!*

Your business is now live in our directory:
...
✅ Customers can now find you by searching: {business_data['keywords'][0]}

💡 *Grow Your Business:*
• Share Telos with customers
• Tell them to search: {business_data['keywords'][0]}
• Need changes? Contact support

🌟 Thank you for choosing Telos! 

Your success is our mission. 🚀"""
        
        resp.message(confirmation)
        
        logger.info(f"New business registered on Telos: {business_data['name']} by {from_number}")
        ...
        if from_number in user_sessions:
            del user_sessions[from_number]
        resp.message("❌ Sorry, there was an error completing your registration. Please try again later or contact Telos support.")
...
def search_businesses(query, resp):
    ...
    try:
        ...
        if unique_results:
            reply = f"🔍 *Telos Search Results* for '{query}':\n\nFound {len(unique_results)} business(es):\n\n"
            ...
            reply += "🌟 *Powered by Telos*\n\n💡 Can't find what you're looking for?\n• Try different keywords\n• Send 'register' to add your business"
        else:
            reply = f"""❌ No businesses found for '{query}' on Telos.
...
🏢 *Own a business?*
Join Telos today - Send 'register' to get started FREE!
...
🌟 Building better business connections."""
        ...
        resp.message("Sorry, Telos encountered a search error. Please try again.")
        return str(resp)
